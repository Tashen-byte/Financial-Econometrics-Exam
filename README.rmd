...
This question makes me think that it is appropriate to make use of PCA, as this will identify which funds are winners and which are losers.
```{r}
Flows <- read_rds("data/ASISA_Flows.rds")
Rets <- read_rds("data/ASISA_Rets.rds")
```
I only want to look at the funds that are present for more than 4 years. 
```{r}
Rets <- Rets %>% 
...
I first need to filter the data set. I think that Returns greater than 15% are high and returns lower than 1% are low.
PCA requires the returns to be in Log form and de-meaned.
```{r}

Rets_log <- Rets %>% 
    group_by(Fund) %>% 
    arrange(date) %>% 
    mutate(Returns = ifelse(Returns > 0.15, 0.15, ifelse(Returns < 0.01, 0.01, Returns))) %>% 
    mutate(Returns = Returns - mean(Returns)) %>% 
    mutate(Returns = log(Returns))
    
```
Using the princomp package.
```{r}
Rets_wide <- Rets_log %>% 
